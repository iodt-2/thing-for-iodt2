...
const CreateTwinThing = () => {
  ...
  // Ontology store — the relationship type vocabulary is published by the
  // backend, so a new type needs no change in this file
  ...
  // Inverse types are generated by the platform, so the user never picks one
  const selectableTypes = relationshipTypes.filter((rt) => !rt.is_derived)
  const defaultRelationshipType = selectableTypes[0]?.name || 'feeds'
  ...
  // Relationship types come from the published ontology, not from this file
  useEffect(() => {
  ...
  const addRelationship = () => {
    setFormData({
      ...
      relationships: [
        ...
        { name: '', target_interface: '', description: '', relationship_type: defaultRelationshipType },
      ],
    })
  }
              ...
              <TabsContent value="relationships" className="space-y-4">
                ...
                {formData.relationships.map((rel, index) => (
                  <div key={index} className="border rounded-md p-3 space-y-2">
                    ...
                    <Select
                      value={rel.relationship_type || defaultRelationshipType}
                    ...
                    >
                      ...
                      <SelectContent className="bg-popover border shadow-md z-50">
                        {selectableTypes.map((relType) => (
                          <SelectItem key={relType.name} value={relType.name}>
                            <span
                              className="inline-block h-2 w-2 rounded-full mr-2 align-middle"
                              style={{ backgroundColor: relType.ui_color }}
                            />
                            <span className="font-mono text-xs font-semibold text-primary mr-2">
                              {relType.name}
                            </span>
                            <span className="text-muted-foreground">
                              {/* Translated where we have a key; the ontology's own
                                  description covers any type added later */}
                              {t(
                                `createThing.rel${relType.name.charAt(0).toUpperCase()}${relType.name.slice(1)}Desc`,
                                { defaultValue: relType.description || '' }
                              )}
                            </span>
                          </SelectItem>
                        ))}
